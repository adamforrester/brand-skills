...
4. Typography — `<h2>Typography</h2>` plus one row per typography token: `<div class="type-row"><span style="font-family: ${withFontFallback(fontFamily)}; font-size: ${fontSize}; font-weight: ${fontWeight}; line-height: ${lineHeight}">The quick brown fox jumps over the lazy dog.</span><span class="type-row-meta">${name} · ${fontSize} / ${fontWeight} · ${fontFamily}</span></div>`. When `tokens/typography.md` frontmatter is empty or all-commented, render `<p class="callout">No typography extracted yet. Run /brand-context:extract.</p>`. **`withFontFallback(fontFamily)`** appends a generic CSS family so the sample degrades to the right class instead of the browser default (serif): if the declared family already contains a comma (an author-supplied stack) or is itself a generic/system keyword (`serif`, `sans-serif`, `monospace`, `inherit`, …), emit it unchanged; if the family is a known serif (Playfair Display, Georgia, Merriweather, Lora, Roboto Slab, EB Garamond, …) append `, serif`; if a known monospace (JetBrains Mono, Fira Code, Roboto Mono, IBM Plex Mono, …) append `, monospace`; otherwise append `, sans-serif`. The `type-row-meta` line shows the **declared** `${fontFamily}` unqualified — the fallback stack appears only in the rendered `style`.
