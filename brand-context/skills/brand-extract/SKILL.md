...
**Pre-filled-from-scope fields:** every prompt below names a **primary key** (the one that satisfies the prompt) and zero or more **additive keys** (multi-URL extras the prompt can also fill). Skip the prompt silently when the primary key is in `filledFromScope` (from `§0a.5`); the additive keys can stay empty without re-prompting. The five primary keys: prompt 1 → `sources.website` (additive: `website_pages`); prompt 2 → `sources.figma` (no additive — entire array is the primary); prompt 3 → any single `sources.social.*` subfield set (additive: the rest); prompt 4 → any single `sources.app_store.*` subfield set (additive: the other); prompt 5 → `sources.design_system_repo` (no additive).

Bail-vs-ask precedence: §0a.5 already enforces the embedded-mode bail before §0d runs — when `interactive_preflight: false` and no required field (per the §0a.5 runtime-required set: `website | figma | brand_guide | screenshots | design_system_repo`) is set, §0a.5 emits the structured error and §0d is unreachable. So in practice every §0d prompt below runs in interactive mode; if you got here, ask conversationally.
...
- Minimally validate each URL: starts with `http://` or `https://`, parses, has a recognizable host. Don't over-validate — bad URLs surface as Stage failures with clear errors later. Silently drop non-URL lines (blanks, comments, prose interjections); if any were dropped, surface a one-line summary (`Dropped 2 non-URL lines from your paste.`) so the practitioner can re-paste if something was meant to be a URL.
...
Auto-classification rules — match by hostname (case-insensitive). Strip leading `www.` AND `m.` subdomains before matching (so `m.facebook.com` and `www.facebook.com` both classify as `facebook.com`). Path is irrelevant — `linkedin.com/company/foo` and `linkedin.com/in/foo` both land at `social.linkedin`. Trailing slashes and query strings are ignored too — match on hostname only.
...
**Conversational fallback.** If the practitioner replies with prose instead of a paste (e.g. "we don't really have social, but the website is example.com"), extract URLs with the same `https?://[^\s]+` match used for paste validation and apply the same auto-classification. Bare-domain mentions (`example.com` without a scheme) are NOT extracted automatically — if the message has only bare-domain mentions, ask the practitioner to paste the full URL(s) once. Don't re-prompt for a clean paste when at least one valid URL was extracted, and don't re-prompt when the practitioner said `skip`.
...
4. Typography — `<h2>Typography</h2>` plus one row per typography token: `<div class="type-row"><span style="font-family: ${withFontFallback(fontFamily)}; font-size: ${fontSize}; font-weight: ${fontWeight}; line-height: ${lineHeight}">The quick brown fox jumps over the lazy dog.</span><span class="type-row-meta">${name} · ${fontSize} / ${fontWeight} · ${fontFamily}</span></div>`. When `tokens/typography.md` frontmatter is empty or all-commented, render `<p class="callout">No typography extracted yet. Run /brand-context:extract.</p>`. **`withFontFallback(fontFamily)`** appends a generic CSS family so the sample degrades to the right class instead of the browser default (serif). Trim the declared family; compare case-insensitively (lowercase) against the lists below. Order of checks: (1) if the family is itself a generic/system keyword, or (2) already contains a comma (an author-supplied stack), emit it **unchanged**; (3) if it is a known serif, append `, serif`; (4) if a known monospace, append `, monospace`; (5) otherwise append `, sans-serif` (the safe default — never serif). These lists must match the CLI's `SERIF_FONTS` / `MONOSPACE_FONTS` / `GENERIC_KEYWORDS` sets verbatim (byte-identical parity depends on the full lists, not an abbreviation):
   - **Generic/system keywords (unchanged):** `serif`, `sans-serif`, `monospace`, `cursive`, `fantasy`, `system-ui`, `ui-serif`, `ui-sans-serif`, `ui-monospace`, `ui-rounded`, `math`, `emoji`, `fangsong`, `inherit`, `initial`, `unset`, `revert`.
   - **Known serif (→ `, serif`):** `playfair display`, `playfair`, `georgia`, `merriweather`, `lora`, `pt serif`, `source serif pro`, `source serif 4`, `noto serif`, `roboto slab`, `slabo 27px`, `libre baskerville`, `crimson text`, `crimson pro`, `eb garamond`, `garamond`, `cormorant`, `cormorant garamond`, `bitter`, `domine`, `zilla slab`, `times new roman`, `times`, `droid serif`, `vollkorn`, `spectral`.
   - **Known monospace (→ `, monospace`):** `jetbrains mono`, `fira code`, `fira mono`, `source code pro`, `ibm plex mono`, `roboto mono`, `space mono`, `inconsolata`, `ubuntu mono`, `courier new`, `courier`, `menlo`, `monaco`, `consolas`, `sf mono`, `cascadia code`, `dejavu sans mono`, `liberation mono`.

   The `type-row-meta` line shows the **declared** `${fontFamily}` unqualified — the fallback stack appears only in the rendered `style`.
...
8. Footer — always renders. Three short paragraphs, each in its own `<p>`: (a) generation timestamp — `Generated ${new Date().toISOString()}.` set at write time; (b) `Source: <code>.brand/</code> directory; regenerate with <code>brand-cli refresh-design</code>.`; (c) the font-fallback caveat, verbatim: `Typography samples use the brand&rsquo;s declared <code>fontFamily</code> with a generic fallback (sans-serif / serif / monospace). If your system doesn&rsquo;t have the font, the sample renders in that generic family &mdash; not the exact brand typeface. The meta line under each sample names the declared family.`
...
**Self-contained constraint.** The output must be a single HTML5 document: starts with `<!DOCTYPE html>`, has exactly one `<style>` block in `<head>`, has zero `<script>` tags, has zero `<link rel="stylesheet">` and zero external `<script src=...>`. No fonts loaded over the network; declared `fontFamily` values are rendered with a generic fallback appended (see §4 `withFontFallback`) and degrade to that generic family class when the exact font is absent.
